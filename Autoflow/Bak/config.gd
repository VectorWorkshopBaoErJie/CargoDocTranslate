...
const TL_file= preload("res://Bak/translation_library_file.gd")
...
func TL_flow_objs_init():
    ...
    for i in TL_flow_objs: ## 迭代每个工作流对象
        var TL_entrys=[]   ## 翻译词条
        for p in i["词条文件"]:
            var _content=G.load_file(Cargo_doc_path+p)
            var _TL_files=TL_file.new(_content,p,Cargo_doc_cn_path)
...
func TL_flow_obj_test(TL_flow_obj):
    var tar_doc_file:String=G.load_file(Cargo_doc_cn_path+TL_flow_obj["源文件名称"])
    var is_pass=true
